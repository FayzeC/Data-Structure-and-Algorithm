...
double postfixEvaluation(char* postfix)
{
    ...
    for(i = 0; i < postfix[i] != '\0'; i++){ // loop through the postfix string
        if(isdigit(postfix[i])){ // if it is an operand
            push(&s, postfix[i] - '0'); // push operand type double into stack
        }else{ // if it is an operator
          operand1 = pop(&s); // pop operand1
          operand2 = pop(&s); // pop operand2
          switch(postfix[i]){ // calculate and store result into stack based on operator
            case '+':
    ...
    while(*postfix){
        if(isdigit(*postfix)){ // if operand push into stack
            push(&s, *postfix - '0');
        }else{ // operator - calculate result
            operand1 = pop(&s);
            operand2 = pop(&s);
            switch(*postfix){
            case '+':
