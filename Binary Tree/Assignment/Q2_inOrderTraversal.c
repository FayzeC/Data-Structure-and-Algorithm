...
void inOrderTraversal(BSTNode *root) {
  ...
  Stack s;
  s.top = NULL;
  // Initialise current node as root and popped to store values popped from stack
  BSTNode *current = root, *popped;

  // If current is null and stack is empty then done
  while (current != NULL || !isEmpty(&s)) {
    while (current != NULL) {
      // Push the current node to S
      push(&s, current);
    ...
    // If current is NULL then do a, b, c
    // a. Pop the top item from stack
    popped = pop(&s);
    // b. Print the popped item
    printf("%d ", popped->item);
    // c. Set current = popped_item->right
