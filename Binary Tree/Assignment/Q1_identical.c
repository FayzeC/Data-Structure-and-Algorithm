...
int identical(BTNode *tree1, BTNode *tree2)
{
    // recursive terminating conditions
    if((tree1 == NULL && tree2 != NULL) || (tree1 != NULL && tree2 == NULL)){
        // 1st condition for not identical: if tree 1 reaches null before tree 2 and vice versa
        return 0;
    }else if(tree1 == NULL && tree2 == NULL){
        // 1st condition for identical: if both trees are empty
        return 1;
    }else if(tree1->item != tree2->item){
        // 2nd condition for not identical: if elements from both trees are not identical
        return 0;
    }else if(tree1->item == tree2->item){
        // 2nd condition for identical: if elements from both trees are identical
        return 1;
    }
