...
void alternateMergeLinkedList(LinkedList *ll1, LinkedList *ll2)
{
    int ll1size = ll1->size, ll2size = ll2->size, index = 1;
    ListNode *temp = ll2->head;;

    if(ll1size >= ll2size){ // add normally when length of ll1 = ll2 or ll1 >ll2
        while(ll2size != 0){
        ...
        removeAllItems(ll2);
    }else if(ll2size > ll1size){ // case where length of l1 < l2
        while(ll1size != 0){
            insertNode(ll1, index, temp->item);
            temp = temp->next;
            index += 2;
