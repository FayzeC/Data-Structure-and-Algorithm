...
{
	// write your code here
	int listIndex = 0, evenIndex = 0, oddIndex = 0;
...
int moveMaxToFront(ListNode **ptrHead){
	// write your code here
    ListNode *pre = NULL, *preMax = NULL, *cur = (*ptrHead);
    int max = cur->num, index = 0;
...
int concatenate(ListNode **ptrHead1, ListNode *head2){
	...
	// if both list are empty, return -1 as error value
	if(*ptrHead1 == NULL && head2 == NULL){
        return -1;
	}

	// if first list is empty, set pointer to list 2
	if(temp == NULL){
        *ptrHead1 = head2;
        return 0;
	}
...
int combineAlternating(ListNode **ptrHead, ListNode *head1, ListNode *head2){
  ...
  // write your code here
  // head1 even index head2 odd index concatenate into ptrhead
  ...
  while(head1 != NULL && head2 != NULL){ // iterates until one of the list ends
  ...
  if(head1 != NULL){ // if list1 is not empty, concatenate the rest of list1
    while(head1 != NULL){
      insertNode(ptrHead, index, head1->num);
      head1 = head1->next;
      index++;
  ...
  if(head2 != NULL){ // if list2 is not empty, concatenate the rest of list2
    while(head2 != NULL){
      insertNode(ptrHead, index, head2->num);
      head2 = head2->next;
      index++;
