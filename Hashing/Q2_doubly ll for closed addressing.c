...
int HashInsert(HashTable *Q1Ptr, int key) {
  ...
  if (Q1Ptr->Table[index].head == NULL) { // empty list
...
int HashDelete(HashTable *Q1Ptr, int key) {
  // Write your code here
  int index = Hash(key, Q1Ptr->hSize);
  ListNode *temp = Q1Ptr->Table[index].head, *prev = NULL;

  if (HashSearch(*Q1Ptr, key) == NULL)
    return 0; // key does not exist

  while (temp != NULL) {
    if (temp->key == key) { // find node to delete
      if (prev == NULL) {   // means it is the first node
...
int Hash(int key, int hSize) { return key % hSize; }
